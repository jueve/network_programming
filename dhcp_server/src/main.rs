...
fn main() {
    ...
    // ヒープ上にDhcpSever構造体を確保し、複数のスレッドから共有するためにArcを利用している
    ...
    match message_type {
        ...
        _ => {
            ...
            Err(failure::format_err!(
                "{:x} received unimpleented message, message_type:{}",
                transaction_id,
                message_type
            ))
        }
...
/**
 * DHCPREQUESTメッセージのオプションにserver_identifierが含まれていない場合のハンドラ
...
fn dhcp_release_message_handler(
    ...
    // 論理削除。DHCPOFFERメッセージを返す際に解放済のIPアドレスを再割当てする場合があるから
...
/**
 * オプションに'requsted IP address'があり、利用可能なればそれを返す
 */
