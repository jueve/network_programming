...
/**
 * DHCPサーバの情報を保持する
 * 複数スレッドで共有されるため、フィールドにmutアクセスする際はロックを取得する必要がある
 * 読み出しだけならフィールドにロックは必要ない
 */
pub struct DhcpServer {
    address_pool: RwLock<Vec<Ipv4Addr>>, // 利用可能なアドレス
    pub db_connection: Mutex<Connection>, // ConnectionはSyncを実装しないのでRwLockはだめ
...
impl DhcpServer {
    ...
    fn init_address_pool(
        ...
        // ネットワーク全てのIPアドレスから、使用されているIPアドレスを除いたものを
        // アドレスプールとする
