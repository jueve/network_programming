...
struct WebServer {
    ...
    connections: HashMap<usize, TcpStream>, // サーバに接続されているクライアントを管理する
...
fn make_response(buffer: &[u8]) -> Result<Vec<u8>, failure::Error> {
    // リクエストラインをパースする
    let http_patern = Regex::new(r"(.*) (.*) HTTP/1.([0-1])\r\n")?;
    let captures = match http_patern.captures(str::from_utf8(buffer)?) {
        Some(cap) => cap,
        None => {
            // 不正なリクエスト
